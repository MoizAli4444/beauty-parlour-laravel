...
@section('content')
<div class="container">
    <h2>{{ isset($staff) ? 'Update' : 'Create' }} Staff</h2>
    ...
    <form action="{{ isset($staff) ? route('staff.update', $staff->id) : route('staff.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="email" name="email" class="form-control" value="{{ old('email', $staff->user->email ?? '') }}">
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                @if(isset($staff) && $staff->image)
                    <img src="{{ asset('storage/' . $staff->image) }}" alt="Profile" width="80">
                @endif
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="working_days[]" class="form-control" multiple>
                    ...
                    @foreach($days as $day)
                        <option value="{{ $day }}" {{ isset($staff) && in_array($day, json_decode($staff->working_days ?? '[]')) ? 'selected' : '' }}>{{ ucfirst($day) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="is_verified" class="form-control">
                    <option value="0" {{ old('is_verified', $staff->is_verified ?? 0) == 0 ? 'selected' : '' }}>No</option>
                    <option value="1" {{ old('is_verified', $staff->is_verified ?? 0) == 1 ? 'selected' : '' }}>Yes</option>
                </select>
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="is_head" class="form-control">
                    <option value="0" {{ old('is_head', $staff->is_head ?? 0) == 0 ? 'selected' : '' }}>No</option>
                    <option value="1" {{ old('is_head', $staff->is_head ?? 0) == 1 ? 'selected' : '' }}>Yes</option>
                </select>
            </div>
